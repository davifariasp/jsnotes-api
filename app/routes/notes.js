...
router.put('/:id', withAuth, async (req,res) =>{
    ...
    try {
        let note = await Note.findById(id) //necessario pegar a nota antiga para verificação

        if(isOwner(req.user, note)){
            const data_att = Date.now
...
router.delete('/:id', withAuth, async (req,res) =>{
    ...
    try {
        ...
        if(isOwner(req.user, note)){
            ...
            res.json({message: 'Note deleted a sucess'}).status(204)
        } else   {
            res.status(403).json({error: 'Permission denied'})
        }
